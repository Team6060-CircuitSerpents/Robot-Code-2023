...
public class Robot extends TimedRobot {
  ...
  DigitalInput dInput = new DigitalInput(0);
  ...
  public Limelight limelight = new Limelight(this, -15, 15);
  ...
  public ArmControl aControl = new ArmControl(2, 1);
  ...
  public void teleopPeriodic() {
    ...
    // Controller 1 sticks
    double lstick2 = control2.getRawAxis(1);
    double rstick2 = control2.getRawAxis(5);

    // Arm Control for controller 1
    boolean lb2 = control2.getRawButton(5);
    boolean rb2 = control2.getRawButton(6);

    double lt2 = control2.getRawAxis(2);
    double rt2 = control2.getRawAxis(3);

    // Arm Extend for controller 1
    boolean X2 = control2.getRawButton(3);
    boolean Y2 = control2.getRawButton(4); 

    boolean A2 = controller.getRawButton(1);
    ...
    if (A || A2){
      ...
      else if (lstick2 > 0.05 || lstick2 < -0.05){
        left = lstick * maxSpeed;
      }
      ...
      else if (rstick2 > 0.05 || rstick2 < -0.05){
        right = rstick * maxSpeed;
      }
    }
    ...
    boolean press = dInput.get();
    if (!press){
      System.out.println("Pressed");
    }
    
    if (lb || lb2){ // this moves the arm up.
      aControl.move(0.15);
    } 
    else if (rb || rb2){ // this moves the arm down.
      aControl.move(-0.15);
    }
    else{ // Stops moving the arm.
      aControl.move(0);
    }

    driveTrain.driveSRX(left*-1, right);
    //driveTrain.driveFX(left*-1, right);
  }
